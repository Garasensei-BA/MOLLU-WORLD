...
CFG = json.loads(
    (ROOT / 'future-sources.json').read_text(encoding='utf-8')
)
...
def community(src):
    ...
    for a in p.links:
        title = ' '.join(a['text']).strip()
        u = urljoin(src['url'], a['href'])
...
def main():
    ...
    result = {
        ...
        'server': 'KR',
        'defaultRangeMonths': 4,
        'supportedRangeMonths': [2, 4, 6, 12],
